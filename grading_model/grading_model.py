...
class FLowModel(nn.Module):
    def __init__(self):
        super().__init__()
        # Given an input returns low level feartures
    ...
    def forward(self, input):
        return self.f_low_seq(input)
    

class FHighModel(nn.Module):
    def __init__(self, num_outputs=5, num_lesions=5):
        super().__init__()
    ...
    def forward(self, f_low, attention_maps=None):
        if attention_maps is None:
            ...
            new_f_low_reshaped = new_f_low.view(batch_size * num_lesions, c, h, w)  # (batch_size*num_lesions, 32, 640, 640)
...
class AttentionModel(nn.Module):
    def __init__(self, num_lesions=4):
        super().__init__()
    ...
    def forward(self, f_low, f_high, masks):
